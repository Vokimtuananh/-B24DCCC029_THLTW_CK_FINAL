...
Nhập liệu: Admin mở Modal "Thêm ngành mới", chọn trường trực thuộc (từ danh sách đã được Redux nạp sẵn), nhập "Mã ngành" và "Tên ngành".
Xác thực (Validation): Khi nhấn "Lưu", form của Ant Design (form.validateFields()) sẽ kiểm tra các quy tắc (rules) như không được để trống.
Kích hoạt sự kiện (Dispatch): Nếu dữ liệu hợp lệ, Component gọi hàm dispatch(createMajor(values)).
